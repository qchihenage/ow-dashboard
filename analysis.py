...
设计说明：
   不同服务器、不同段位的英雄强弱不同（打法差异很大），
   因此「强度评分与梯度」在每个 服务器×段位 内独立计算，
   看板切换服务器/段位时排行联动更新。
...
def analyze():
    ...
    regions = sorted({col(r, "region") for r in rows})
    ...
    for region in regions:
        for tier in tiers:
            ...
            region_data[region][tier] = entries

    # 历史趋势：hero_id -> region -> tier -> [ {date, winrate, pickrate, banrate} ]
    history = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    for r in rows:
        hid, region, tier = col(r, "hero_id"), col(r, "region"), col(r, "tier")
        history[hid][region][tier].append({
    ...
    for hid in history:
        for region in history[hid]:
            for tier in history[hid][region]:
                history[hid][region][tier].sort(key=lambda d: d["date"])
...
def main():
    ...
    meta = result["meta"]
    n_combos = sum(len(t) for t in result["region_data"].values())
    print(f"完成！赛季={meta['season']} | 服务器={meta['regions']} "
          f"| 段位数={len(meta['tiers'])} | 英雄数={len(result['heroes'])} "
          f"| 组合数={n_combos}")
    print(f"输出文件: {OUTPUT_JSON}")

    # 打印一段简要排行，便于快速核对（默认显示亚洲服全段位）
    first_region = meta["regions"][0] if meta["regions"] else ""
    if "All" in result["region_data"].get(first_region, {}):
        print(f"\n{first_region} 服 · 全段位强度排行 Top 10：")
        for e in result["region_data"][first_region]["All"][:10]:
